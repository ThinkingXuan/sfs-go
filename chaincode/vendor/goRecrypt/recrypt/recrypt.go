...
func encryptKeyGen(pubKey *ecdsa.PublicKey) (capsule *Capsule, keyBytes []byte, err error) {
	...
	// generate aes key
	keyBytes, err = util.Sha3Hash(curve.PointToBytes(point))
	if err != nil {
		return nil, nil, err
	}
	capsule = &Capsule{
...
func RecreateAESKeyByMyPriKeyStr(capsule *Capsule, aPriKeyStr string) (keyBytes []byte, err error) {
	aPriKey, err := util.PrivateKeyStrToKey(aPriKeyStr)
	if err != nil {
		return nil, err
	}
...
func EncryptByStr(message, pubKeyStr string) (cipherText []byte, capsule *Capsule, err error) {
	key, err := util.PublicKeyStrToKey(pubKeyStr)
	if err != nil {
		return nil, nil, err
	}
...
func ReKeyGenByStr(aPriKeyStr, bPubKeyStr string) (*big.Int, *ecdsa.PublicKey, error) {
	...
	bPubKey, err := util.PublicKeyStrToKey(bPubKeyStr)
	if err != nil {
		return nil, nil, err
	}
...
func DecryptByStr(bPriKeyStr string, capsule *Capsule, pubXStr string, cipherText []byte) (plainText []byte, err error) {
	bPriKey, err := util.PrivateKeyStrToKey(bPriKeyStr)
	if err != nil {
		return nil, err
	}
	pubX, err := util.PublicKeyStrToKey(pubXStr)
	if err != nil {
		return nil, err
	}
